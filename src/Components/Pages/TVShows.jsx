...
const TVShows = () => {
    ...
    const fetchPopular = async () => {
        await axios.get(requests.tvShows).then(resp => {
      ...
      <div className='md:py-10 md:px-14 p-8'>
          <div className='mt-14'>
              <motion.h1
                  ...
                  className='text-white md:text-lg text-sm font-bold flex items-center gap-1'>Popular TV Shows  <Emoji Emoji={Emojies.okHand} width={'6'} height={'6'} />
              </motion.h1>
                      ...
                      tvShows.map(res => (
                          <div
                              ...
                              <div className='absolute w-full h-full bg-gradient-to-t from-black to-transparent z-10 top-0 left-0 rounded-md p-4 flex items-end justify-between '>
                                  <div>
                                              <h1 className='flex flex-col justify-end text-sm lg:text-lg text-white font-bold w-full' >{ truncateString(res?.name , 30) }</h1>
                                  <p className='text-[#c1c1c1] text-sm lg:text-[1rem]'>{res?.first_air_date}</p>
                                  </div>
